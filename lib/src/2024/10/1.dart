...
class Puzzle2024101 extends PuzzlePart {
  @override
  Map<String, String?> get dependentTestFiles =>
      {'test_input': 'test_output', 'input': 'output'};

  @override
  PuzzleIdentifier get id => PuzzleIdentifier(2024, 10, 1);
  ...
  PuzzleOutput run(String input) {
    ...
    Set<({Pos start, Pos end})> positions = {};
    ...
    while (possibleTrails.isNotEmpty) {
      for (final possibleTrail in possibleTrails) {
        if (possibleTrail.length == 10) {
          positions.add((start: possibleTrail.first, end: possibleTrail.last));
          continue;
        }
    ...
    return PuzzleOutput(positions.length.toString());
  }
}
